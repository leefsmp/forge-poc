...
export default function(viewer) {
  ...
  var _names = ["Animation.Viewing.Snapper.Tool"];
  var _active = false;

  var _faceOverlayName = 'SnapperTool-snapper-face';
  var _vertexOverlayName = 'SnapperTool-snapper-vertex';
  var _edgeOverlayName = 'SnapperTool-snapper-edge';
  ...
  this.handleMouseMove = function (event) {
    ...
    if (!_isDragging) {
      ...
      if (_snappingNode) {
        ...
        _viewer.impl.matman().highlightObject2D(
          _snappingNode.dbId, false);
      }
      ...
      if (_viewer.impl.renderer().hasMRT()) {

        var result = _viewer.impl.snappingHitTest(
          event.canvasX,
          event.canvasY,
          false);
      }
        ...
        var result = _viewer.impl.hitTest(
          event.canvasX,
          event.canvasY,
          false);
      }
  ...
  this.enable = function(enable) {
    ...
    enable ?
      viewer.toolController.activateTool(name) :
      viewer.toolController.deactivateTool(name)
  }
